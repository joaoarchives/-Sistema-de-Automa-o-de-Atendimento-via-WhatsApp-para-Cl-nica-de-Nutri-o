...
def listar_conversas():
    ...
    from database.mensagens import listar_conversas as _listar
    return jsonify({"conversas": _listar()})


@api.route("/conversas/<telefone>", methods=["GET"])
@token_required
def mensagens_paciente(telefone):
    """Retorna todas as mensagens de um paciente."""
    from database.mensagens import listar_mensagens_paciente
    return jsonify({"mensagens": listar_mensagens_paciente(telefone)})
