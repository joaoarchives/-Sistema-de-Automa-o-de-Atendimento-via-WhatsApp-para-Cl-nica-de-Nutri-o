...
for _env_path in (_BASE_DIR / ".env", Path(__file__).resolve().parent / ".env"):
    if _env_path.exists():
...
from database.mensagens import atualizar_status_whatsapp, salvar_log_whatsapp, salvar_mensagem_recebida
        ...
        for entry in entries:
            for change in entry.get("changes", []):
                ...
                for message in value.get("messages", []):
                    ...
                    try:
                        salvar_mensagem_recebida(from_number, texto, msg_id)
                    except Exception:
                        logger.exception("Erro ao salvar mensagem recebida")
